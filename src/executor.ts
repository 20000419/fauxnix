...
function emitToPrepDest(dest: PrepDest, msg: string, fallback: (s: string) => void): void {
  if (dest.kind === 'nul') return;
  if (dest.kind === 'file') {
    try {
      appendRedirectText(dest.path, msg);
      return;
    } catch {
...
async function runPlans(
  ...
  for (const plan of plans) {
    ...
    const emitPrepError = (msg: string) => emitToPrepDest(prepStderr, msg, (s) => { stderr += s; });
