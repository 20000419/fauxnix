...
const FX_TNK_FN = [
  ...
  '    elseif ($op -eq [char]47) { if ($r -eq 0) { if ($script:fx_skip) { $v = 0 } else { throw "division by 0" } } else { $fx_rem = [long]0; $v = [math]::DivRem([long]$v, [long]$r, [ref]$fx_rem) } }',
...
function testBinaryExpr(l: Word, op: string, r: Word, allowKsh: boolean): string {
  ...
  const le = stringOperandExpr(l);
  const re = stringOperandExpr(r);
