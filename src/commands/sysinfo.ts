...
function testVExpr(w: Word): string {
  ...
  if (
    ...
    return '$true';
  if (name === 'OLDPWD') return "[bool]$env:FAUXNIX_OLDPWD";
  return "(Test-Path -LiteralPath ('Env:' + [string](" + exprOfWord(w) + ')))';
}
...
const FX_GMATCH_FN = [
  ...
  '    if ($c -eq [char]92) {',
  '      if (($pi + 1) -ge $p.Length) {',
  '        if ($si -ge $s.Length -or $s[$si] -ne [char]92) { return $false }',
  '        $si++; $pi++; continue',
  '      }',
  '      $n = $p[$pi + 1]',
  "      if (@('*','?','[',']','\\') -contains [string]$n) {",
  '        if ($si -ge $s.Length -or $s[$si] -ne $n) { return $false }',
  '        $si++; $pi += 2; continue',
  '      }',
  '      if ($si -ge $s.Length -or $s[$si] -ne [char]92) { return $false }',
  '      $si++; $pi++; continue',
  ...
  '      }',
  '      if ($j -ge $p.Length) {',
  "        if ($si -ge $s.Length -or $s[$si] -ne '[') { return $false }",
  '        $si++; $pi++; continue',
  '      }',
  '      if ($si -ge $s.Length) { return $false }',
  ...
  '      $si++; $pi = $j + 1; continue',
  '    }',
  '    if ($si -ge $s.Length -or $s[$si] -ne $c) { return $false }',
  '    $si++; $pi++; continue',
  '  }',
  '  return ($si -eq $s.Length)',
  '}',
  'function fx-gmatch($a, $b) { return [bool](fx-gok ([string]$a) 0 ([string]$b) 0) }',
].join('\n');
