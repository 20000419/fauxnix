  ...
  return sets
    .map(
      (s) =>
        ...
        s.name.replace(/'/g, "''") +
        "' }) -join ';'); " +
        emitSetValPut(s.name, exprOfWord(s.value)),
    )
    .join('\n');
};
...
const FX_TNK_FN = [
  ...
  '    }',
  '  }',
  "  if (($null -eq $fx_src) -and (@($env:FAUXNIX_SETVARS -split ';' | Where-Object { $_ -ceq $nm }).Count -gt 0)) { $fx_src = [Environment]::GetEnvironmentVariable([string]$nm) }",
  ...
  '  return [long]$script:fx_as.Substring($start, $script:fx_ai - $start)',
  '}',
  'function fx-apow {',
  '  $v = fx-anum',
  '  fx-askip',
