...
const FX_TNK_FN = [
  ...
  '    return $v',
  '  }',
  '  if ($script:fx_ai -lt $script:fx_as.Length) {',
  '    $c0 = [int]$script:fx_as[$script:fx_ai]',
  '    if (($c0 -ge 65 -and $c0 -le 90) -or ($c0 -ge 97 -and $c0 -le 122) -or $c0 -eq 95) {',
  ...
  '      }',
  '      $nm = $script:fx_as.Substring($ns, $script:fx_ai - $ns)',
  '      if ($script:fx_skip) { if ($neg) { return 0 }; return 0 }',
  '      $fx_ev = Get-ChildItem Env: | Where-Object { $_.Name -ceq $nm } | Select-Object -First 1',
  '      if (-not $fx_ev) { if ($neg) { return 0 }; return 0 }',
  '      $saveAs = $script:fx_as; $saveAi = $script:fx_ai; $saveSkip = $script:fx_skip',
  '      $vv = fx-arith $fx_ev.Value',
  ...
  '    elseif ($op -eq [char]47) { if ($r -eq 0) { if ($script:fx_skip) { $v = 0 } else { throw "division by 0" } } else { $v = [long][math]::Truncate([double]$v / $r) } }',
  '    else { if ($r -eq 0) { if ($script:fx_skip) { $v = 0 } else { throw "division by 0" } } else { $v = $v % $r } }',
  '  }',
  '  return $v',
  '}',
  'function fx-aadd {',
...
const FX_ENVGET_FN = [
  ...
  '  $n = [string]$n',
  "  if ($n -eq 'HOME') { return [string]$HOME }",
  "  if ($n -eq 'PWD') { return [string]$PWD.Path }",
  "  if ($n -eq 'USER' -or $n -eq 'LOGNAME') { return [string]$env:USERNAME }",
  "  if ($n -eq 'PATH') { return [string]$env:PATH }",
  "  if ($n -eq 'SHELL') { return 'powershell' }",
  "  if ($n -eq 'TERM') { return 'xterm-256color' }",
  "  if ($n -eq 'OLDPWD') { return [string]$env:FAUXNIX_OLDPWD }",
  "  if ($n -eq '?') { return [string]$fx_prev }",
  "  if ($n -eq '$') { return [string]$PID }",
  "  if ($n -eq 'HOSTNAME') { return [string]$env:COMPUTERNAME }",
...
const FX_FCOMP_FN = [
  ...
  '  } catch { return $false }',
  '  if ($ia.FullName -ceq $ib.FullName) { return $true }',
...
function buildTest(ws: Word[], label: string, allowRe = false): string {
  ...
  if (res.expr && res.expr.indexOf('fx-envget') >= 0) helpers.push(FX_ENVGET_FN);
  if (
    res.expr &&
