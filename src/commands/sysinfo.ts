...
const FX_TNK_FN = [
  'function fx-arith($s) {',
  '  $t = [string]$s',
  "  $t = [regex]::Replace($t, '(?<![0-9A-Fa-fxX])[A-Za-z_][A-Za-z0-9_]*', {",
  '    param($m)',
  "    if ($m.Value -eq 'x' -or $m.Value -eq 'X') { return $m.Value }",
  ...
  "  if ($t -notmatch '^[0-9+\\-*/%() \\t]+$') { throw 'integer expression expected' }",
...
const FX_POSIXRE_FN = [
  ...
  '  $i = 0',
  "  $keep = @('.','[',']','\\','(',')','*','+','?','{','}','|','^','$','1','2','3','4','5','6','7','8','9','b','B','w','W','s','S','z')",
  '  while ($i -lt $t.Length) {',
  '    $c = $t[$i]',
  "    if ($c -eq '\\') {",
