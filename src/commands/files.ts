...
const ls: Handler = (args) => {
  ...
  return [
    ...
    '}',
    'function fx-name($it) {',
    '  $n = $it.Name; if ($fx_operandMode -and $it.FauxnixOperand) { $n = $it.FauxnixOperand }',
    ...
    '    }',
    '    $fx_entry = Get-Item -LiteralPath $fx_g -Force',
    '    $fx_entry | Add-Member -NotePropertyName FauxnixOperand -NotePropertyValue ([string]$fx_g) -Force',
    '    $fx_all += ,$fx_entry',
    '  }',
    '}',
    'function fx-render-ls-group($fx_show, $fx_operandMode) {',
    ...
    '}',
    '}',
    'if (' + (dirOnly ? '$true' : '$false') + ') { fx-render-ls-group $fx_all $true }',
    'else {',
    '  $fx_files = @($fx_all | Where-Object { -not $_.PSIsContainer })',
    '  if ($fx_files.Count -gt 0) { fx-render-ls-group $fx_files $true }',
    '  $fx_directories = @($fx_all | Where-Object { $_.PSIsContainer } | Sort-Object -Property FauxnixOperand)',
    ...(reverse ? ['  [array]::Reverse($fx_directories)'] : []),
    '  $fx_needSeparator = $fx_files.Count -gt 0',
    '  foreach ($fx_directory in $fx_directories) {',
    '    if ($fx_operandCount -gt 1) {',
    "      if ($fx_needSeparator) { '' }",
    "      ([string]$fx_directory.FauxnixOperand) + ':'",
    '    }',
    '    $fx_children = @(Get-ChildItem -LiteralPath $fx_directory.FullName -Force:' + (all || almost ? '$true' : '$false') + ')',
    '    fx-render-ls-group $fx_children $false',
    '    $fx_needSeparator = $true',
    '  }',
    '}',
  ].join('\n');
};
