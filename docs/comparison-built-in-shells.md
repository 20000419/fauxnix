...
| Capability | Claude Code `Bash` | Codex shell | OpenCode built-in | fauxnix |
|---|---|---|---|---|
| What it actually is | Git Bash (requires Git for Windows) | **Windows PowerShell 5.1** | Git Bash (if bash.exe on PATH) | translated PowerShell |
| Commands authored in | bash | **PowerShell** | bash | bash |
| `cd` persists across calls | ✔ (own snapshot/restore) | ✘ | ✘ | ✔ |
| Sees full Windows process table | ✘ (MSYS procs only: 24) | ✔ (447) | ✘ (18) | ✔ (485) |
| `grep 连接` on a GBK file | ✘ 0 matches | ✘ (path broke first) | ✘ 0 matches | ✔ 1 match |
...
| Error style shown to model | bash | localized PowerShell (e.g. zh-CN) | bash | normalized English bash |
| Extra install needed | Git for Windows | none | bash on PATH | `npm i -g fauxnix-cli` |
