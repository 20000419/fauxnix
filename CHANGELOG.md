...
- **C-1 complete**: `while`/`until` loops (#182) and `case ... esac` with
  pattern alternatives `a|b)`, `*` default, `;;` only — `;&`/`;;&` fallthrough
  fails loud with an alternative (#190)
- **C-2**: array assignment `A=(x y z)` persisting via the FAUXNIX_ARRS
  sidecar; `${name//pat/str}` global/first replace and `${name:off:len}`
  slices including negative offsets (#191)
- **C-3**: session-scoped positional parameters — `set -- a b`, `$1`..`$n`,
  `$#`, `"$@"`, `shift`, `$0` = tool name, reset clears (per the accepted
  mini-RFC) (#185)
- **U-1**: `fauxnix install --claude/--codex/--opencode/--kimi/--qwen` —
  idempotent, preserves unrelated config keys, refuses invalid JSON instead
  of clobbering (#183)
- **U-3**: every loud-reject carries a one-line alternative (grep-enforced
  in CI) (#184)
- **U-4**: harness quickstarts docs/examples/ x5 (#187)
- **U-8**: perf guard in CI — warm p50 <50ms, first-frame <400ms (#181)
- **C-5**: archive family (gzip/gunzip/zcat/zip/unzip) spec'd (#188)
- **C-7**: corpus grown to 126 cases; weekly oracle workflow (#189)
- Integration fixes by maintainer: isAmpWord accepted only WORD-typed '&'
  (fallthrough detection never fired); specs doc regenerated byte-exact;
  U-3 tests updated for implemented constructs

319 tests green incl. 126-case Git Bash differential corpus (≥95% gate).
## v0.10.0 — 2026-09-02
